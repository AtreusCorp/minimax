...
class StrategyMinimaxMyopic(Strategy):
    ...
    def suggest_move(self, state):
        ...
        move_dict = gen_move_dict(state, 4)
...
def gen_move_dict(state, n):
    ''' (GameState, str, int) -> dict of {obj: float}

    Precondition: n > 0

    Return a dictionary with the indices of each move in
    state.possible_next_moves() as keys and the score of next_player
    after applying the corresponding move. Estimate the score of next_player
    after looking n moves ahead.
    ...
    if not possible_moves:
        ...
        # Estimate the score
        return {None: state.rough_outcome()}
        ...
        for move_index in range(possible_move_length):
            ...
            # Decrease the number of moves to look ahead by by 1
            temp_dict = gen_move_dict(applied_state, n - 1)
